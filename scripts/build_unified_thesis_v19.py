"""
⚠️ SUPERSEDED — 이 빌드 파이프라인은 더 이상 논문 최종본을 만들지 않습니다 (2026-07-17).

논문 최종본은 저장소 밖의 한글 문서 `260716_박사학위논문_신동욱.hwpx` 이며,
이후의 심사 반영 사항은 hwpx 에만 있습니다. 이 스크립트로 재생성하면
최신 작업이 반영되지 않은 예전 원고가 나옵니다.

용어 주의: 아래에서 참조하는 원고(thesis_current/v5_rewrite/*.md)는 이전 명칭
L-DWA 를 사용합니다. 최종본의 명칭은 A-DWA 입니다. 파일명 `05_L-DWA.md` 는
실제 파일 경로이므로 유지합니다. 자세한 내용은 thesis_current/README.md 참조.
